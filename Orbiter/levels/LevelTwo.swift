...
class LevelTwo: SKScene {
    ...
    override func didMove(to view: SKView) {
        ...
        if let earthNode : Mass = self.childNode(withName: "earthNode") as? Mass {
            //make the declared variable equal to earthNode
            self.earth = earthNode
        }
        //make sure the child actually is of the Mass class
        if let moonNode : Mass = self.childNode(withName: "moonNode") as? Mass {
            //make the declared variable equal to earthNode
            self.moon = moonNode
        }
        ...
        self.earth?.id = 1
        //initialize non-default moon values
